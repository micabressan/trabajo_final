...
if (subcomp(link_target, NULL)) {
	...
	d_log("audioSort:   Sorting mp3 THE WEIRD WAY\n");
	if (*info->id3_artist) {
		d_log("audioSort:     - artist: %s\n", info->id3_artist);
		if (!strncasecmp(link_target, "VA", 2) && (link_target[2] == '-' || link_target[2] == '_'))
			memcpy(info->id3_artist, "VA", 3);
		if (memcmp(info->id3_artist, "VA", 3)) {
			...
			//need to reserve the right amount of memory, so char + artist name
			temp_p = ng_realloc(temp_p, 2+strlen(info->id3_artist)+1, 1, 0, NULL, 1);
			//gotta give length in this for some reason.. might not work this way
			snprintf(temp_p, 2+strlen(info->id3_artist)+1, "%c/%s", toupper(*link_target),info->id3_artist);
			//checks for whatever reason, left them in
			temp_p = check_nocase_linkname(audio_artist_path, temp_p);
			space_to_dot(temp_p);
