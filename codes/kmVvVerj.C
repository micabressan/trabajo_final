...
/*                                                                            */
/* Reseni vytvoril(a) a odevzdava: (Marek Pakes, xpakes00)                    */
/*                                                                            */
/******************************************************************************/
...
void ADC0_Init(void)
{
	...
	ADC0_CFG1 = 0b01100001;		// (Bus clock)/2 (0b01)
								// clock divider = 8 (0b11xxxxx);

	ADC0_SC3 = 0b00001111;		// 32 samples averaged(0b11)
								// hardware average enable (0b1xx)
								// continuous conversion enable (0b1xxx)

	ADC0_SC1A = 0b01000110;		// input channel(AD6 -> 0b01100)
								// enable interrupt(0b1xxxxxx);
								// conversion complete(0b0xxxxxxx);
}
...
void ADC0_IRQHandler(void) {
	unsigned int ADC0_RA_cpy = ADC0_RA;
	unsigned int RAxV = ADC0_RA_cpy * 3300;
	unsigned int my_result = RAxV / 255;

	sprintf(result, "%d", my_result);
}
/******************************************************************************/
